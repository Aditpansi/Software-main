...
async function fetchData() {
  try {
    ...
    // Map available device IDs to HTML cells
    const deviceIds = Object.keys(data);
    deviceIds.forEach((id, index) => {
      const htmlId = index + 1; // map first device to LoadCell1, etc.
      const element = document.getElementById(`loadCellValue${htmlId}`);
      if (element) {
...
// Position 30 load cells in perfect circle
        ...
        const totalCells = 30;
        const outerRadius = 310; // Distance for load cells
